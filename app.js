...
function cleanReply(t){
  ...
  // + vorbarie de planificare in engleza ("We need to...", "Let's craft...", "Must not ask...")
  const planTalk=/the user\b|they (ask|want|said)|we (need|must|should|will|have)|let'?s |let me |must not|should not|need to |have to |no english|in (romanian|english)|the (rules|instructions)|user (wants|asks|said)|craft a|avoid using|\bensure\b|double-check|check for|for example|such as|\be\.g\.|don't (use|include|mention)|do not (use|include)|be careful|no intro\b|narrat(e|ing)|thinking out loud|step-by-step plan/i;
  const lines=t.split("\n");
  const kept=lines.filter(l=>!meta.test(l)&&!planTalk.test(l));
  // daca totul era doar vorbarie (fara 🍳 deloc), intoarce "" => se incearca urmatorul model
  if(kept.join("").indexOf("🍳")<0&&lines.length-kept.length>=2)return "";
  t=kept.join("\n").replace(/\n{3,}/g,"\n\n").trim();
  return t;
}
